...
/* ============================================================
   GLITCH TEXT
   Inline for this section — full standalone version in Phase 9.
   ============================================================ */
function GlitchHeading({ text }) {
    ...
    return (
        <h2
            className={`
        font-space font-black text-title
        relative select-none
        ${glitching ? 'anim-rgb-shift' : ''}
      `}
...
/* ============================================================
   ERROR RAIN
   Performant CSS-animated falling error messages.
   ============================================================ */
function ErrorRain({ intensity = 1 }) {
    const prefersReduced = useReducedMotion()

    const rainItems = useMemo(() => {
        const count = prefersReduced ? 0 : Math.floor(20 * intensity)
...
/* ============================================================
   SQUASHABLE BUG
   Click to squash — but 2 more appear.
   ============================================================ */
...
function Bug({ id, x, y, onSquash }) {
    ...
    return (
      ...
      "
        ...
        >
            {/* Bouncing click arrow above — always visible, not just on hover */}
            {!squashed && (
                <motion.span
                    animate={{ y: [0, -5, 0] }}
            ...
            {/* SQUASH label — always visible, no longer hidden on mobile */}
            {!squashed && (
                <span
                    className="
...
function BugPlayground() {
    ...
    const [hasSquashedAny, setHasSquashedAny] = useState(false)   // ← NEW
    ...
    return (
      ...
      ">
                ...
                >
                    {bugs.length > 15 ? '⚠ containment failing' : !hasSquashedAny ? '← click a bug!' : 'keep squashing'}
                </span>
            </div>
        ...
        >
            ...
            <div className="flex items-start justify-between gap-3 mb-3">
                <div className="flex items-center gap-2 min-w-0">
                    ...
                    />
                    {/* Commit hash */}
                    <code className="font-mono text-xs text-white/30 flex-shrink-0">
                        {commit.commit}
                    </code>
                    {/* Time */}
          ...
          `}
            ...
            {/* Narrator note */}
            <p className="font-inter text-xs text-white/40 italic mb-3">
                {commit.note}
            </p>
...
function BugTimeline({ isVisible }) {
    return (
        <div
            className="relative"
            role="list"
            aria-label="Bug fix attempt timeline"
        >
          ...
          w-px
          bg-gradient-to-b from-neon-red/60 via-neon-red/20 to-transparent
          ml-[7px] sm:ml-0 sm:left-1/2 sm:-ml-px
          hidden sm:block
        "
                aria-hidden="true"
                style={{
...
/* ============================================================
   SHAKE WRAPPER
   Triggers CSS shake on scroll entry.
   ============================================================ */
function ShakeOnEntry({ children, className = '' }) {
    ...
    return (
        <div
            ref={ref}
            className={`${className} ${shaking && !prefersReduced ? 'anim-shake' : ''}`}
        >
            {children}
        </div>
    )
        ...
        <div className="space-y-4" role="region" aria-label="Console.log detective — find the bug">
            <div className="space-y-1">
                ...
                <h3 className="font-space text-xl sm:text-2xl font-bold text-white">
                    Find the bug{' '}
                    <span className="text-neon-red">with console.log</span>
                </h3>
            ...
            <div className="terminal-window rounded-xl overflow-hidden">
                    ...
                    {SUSPECT_LINES.map((line, i) => (
                        <div key={line.id}>
                            <motion.div
                                ...
                                className={`
                  group relative flex items-start gap-3 px-3 py-1.5 rounded
                ...
                `}
                  ...
                  ${revealed[line.id] && line.isBug ? 'text-neon-red/90' : 'text-white/70'}
                `}>
                                    {line.code}
                                </span>
                            ...
                            <AnimatePresence>
                                {revealed[line.id] && (
                                    ...
                                    >
                                        <div className={`
                      ml-7 mr-3 mb-1 px-3 py-2 rounded border-l-2
                      ${line.isBug ? 'bg-neon-red/8 border-neon-red/50' : 'bg-bg-void/60 border-neon-green/20'}
                    `}>
  ...
  return (
    <div className="space-y-4" role="region" aria-label="Stack trace decoder — click to translate">
      <div className="space-y-1">
        ...
        <h3 className="font-space text-xl sm:text-2xl font-bold text-white">
          What does this{' '}
          <span className="text-neon-red">actually mean?</span>
        </h3>
        <p className="font-inter text-sm text-white/40">
...
export default function Section3_DebuggingAbyss() {
  ...
  useGSAP(() => {
    ...
    gsap.timeline({
      scrollTrigger: {
        ...
        onEnter: () => {
          setShakeTitle(true)
          setTimeout(() => setShakeTitle(false), 700)
        },
      },
    })
  ...
  /* ── GSAP: Section background color scrub ── */
  useGSAP(() => {
    if (prefersReduced) return
    ...
    >
      ...
      >
        ...
        >
          ...
          <p className="s3-subtitle font-inter text-base sm:text-xl text-white/50 max-w-xl leading-relaxed">
            It's never just{' '}
            <code className="
              font-mono text-neon-red bg-neon-red/10
              border border-neon-red/20 px-1.5 py-0.5 rounded text-sm
            ">
              a missing semicolon.
            </code>
          ...
          <div
            className="
              s3-tagline glass-card-neon border-neon-red/20
              rounded-2xl px-6 sm:px-10 py-5
              max-w-lg space-y-2
            "
          >
            ...
            <blockquote className="font-space text-lg sm:text-2xl font-bold text-white">
              "I'll just{' '}
              <span className="text-glow-red">debug this quickly</span>.
              Give me 5 minutes."
            </blockquote>
      ...
      >
        ...
        <motion.div
          ...
          className="
            text-center glass-card-neon border-neon-red/20
            rounded-2xl p-6 sm:p-8 max-w-2xl mx-auto space-y-3
          "
        >
